...
class WDLoss(nn.Module):
    ...
    def forward(self, real_x, fake_x):
        e_real = self.discriminator(real_x)
        e_fake = self.discriminator(fake_x)
        grad_penalty = self._calc_gradient_penalty(real_x, fake_x)
        w_distance = torch.mean(e_real) - torch.mean(e_fake)
...
class CTGANLoss(nn.Module):
    ...
    def forward(self, x, m, hint, c, mask_c):
        input_g = torch.cat(dim=1, tensors=[x, m, c])
        imputed = self.generator(input_g)
        imputed = imputed * (1 - m) + x * m
        loss_g = self._cond_loss(imputed, c, mask_c)
        input_d = torch.cat(dim=1, tensors=[imputed, hint, c])
        estimate = self.discriminator(input_d)
        loss_g += -torch.mean(estimate)
        return loss_g

    def _cond_loss(self, imputed_x, c, m_c):
        loss = []
        st = 0
        st_c = 0
        ...
        return (loss * m_c).sum() / imputed_x.shape[0]
